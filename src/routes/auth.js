...
async function bloqueado(email, ipAddr) {
  const r = await db(
    `SELECT COUNT(*) AS n FROM tentativas WHERE email=$1 AND ip=$2 AND ok=false AND criado_em > NOW()-INTERVAL '15 minutes'`,
    [email, ipAddr]
  );
  return parseInt(r[0].n) >= 5;
}
...
router.post('/cadastro', async (req, res) => {
  try {
    ...
    const existe = await db.one('SELECT id FROM graficas WHERE email=$1', [email.toLowerCase()]);
    ...
    await db(`INSERT INTO logs (grafica_id,tipo,descricao,ip) VALUES ($1,'cadastro','Nova conta criada',$2)`, [g.id, ip(req)]);
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (await bloqueado(email.toLowerCase(), ipAddr)) {
      await db('INSERT INTO tentativas (email,ip,ok) VALUES ($1,$2,false)', [email.toLowerCase(), ipAddr]);
    ...
    if (!g.ativo) return res.status(403).json({ erro: g.motivo_bloqueio ? `Bloqueado: ${g.motivo_bloqueio}` : 'Acesso bloqueado. Contate o suporte.' });
    ...
    await db(`INSERT INTO logs (grafica_id,tipo,descricao,ip) VALUES ($1,'login','Login realizado',$2)`, [g.id, ipAddr]);
